...
to update-plot ;; plotting procedure
  ...
  ;; plotting the mean number of short-term sexual partners among single and coupled individuals who've ever had short-term mating
  set-current-plot "Sexual partner 3"
  ifelse (count turtles with [ coupled? = false and short-term-count > 0 ] != 0)
  and (count turtles with [ coupled? = true and short-term-count > 0 ] != 0) [
    set-current-plot-pen "Single"
    plot ( sum [number-of-partner-short] of turtles with [ coupled? = false ] ) /
      ( count turtles with [ coupled? = false and short-term-count > 0 ] )
    set-current-plot-pen "Coupled"
    plot ( sum [number-of-partner-short] of turtles with [ coupled? = true ] ) /
      ( count turtles with [ coupled? = true and short-term-count > 0 ] )
    ] [
    set-current-plot-pen "Single"
    plot 0
    set-current-plot-pen "Coupled"
...
"" ""
PENS
"Single" 1.0 0 -13791810 true "" ""
"Coupled" 1.0 0 -5825686 true "" ""
...
1344
508
The mean number of short-term sexual partners average over single and coupled individuals who've ever had short-term mating
11
0.0
1
